...
from phigros_publisher.storage_check import verify_conditional_writes, verify_object_copy


class ProbeStore:
    def __init__(self):
        self.objects = {"phigros/current.json": b"active", "phigros/releases/old/music.ogg": b"music"}
        self.puts = []
        self.deletes = []
        self.reads = []
        self.bodies = []
...
class StorageCheckTests(unittest.TestCase):
    ...
    def verify(self):
        return verify_conditional_writes(self.store, "bucket", "phigros")

    def assert_only_probe_was_touched(self):
        for key, _body, _options in self.store.puts:
            self.assertRegex(key, r"^phigros/publisher-checks/[0-9a-f]{32}$")
        self.assertTrue(all(key.startswith("phigros/publisher-checks/") for key in self.store.reads + self.store.deletes))
    ...
    def test_existing_probe_is_not_deleted_when_initial_create_fails(self):
        nonce = "a" * 32
        key = f"phigros/publisher-checks/{nonce}"
        self.store.objects[key] = b"existing unrelated probe"
        with patch("phigros_publisher.storage_check.uuid4", return_value=SimpleNamespace(hex=nonce)):
            with self.assertRaisesRegex(RuntimeError, "能力检查失败"):
                self.verify()
    ...
    def test_copy_probe_copies_exact_bytes_and_cleans_both_keys(self):
        result = verify_object_copy(self.store, "bucket", "phigros")
        self.assertTrue(result["objectCopy"])
        self.assertEqual(self.store.objects, self.original)
        self.assertEqual(len(self.store.copies), 1)
        self.assertEqual(len(self.store.deletes), 2)
        self.assertTrue(all(key.startswith("phigros/publisher-checks/") for key in self.store.deletes))
    ...
    def test_copy_unsupported_aborts_and_cleans_source(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "CopyObject"):
            verify_object_copy(self.store, "bucket", "phigros")
        self.assertEqual(self.store.objects, self.original)
        self.assertEqual(len(self.store.deletes), 1)
